...
% curva che determina la forma del becco
x = 0.9*(1 + [0, 0.4, 1, 1.6, 2.2, 2.8, 3.4, 3.4])'; % parte quasi stazionario, poi accelera avanti
y = ones(size(x));
z = [1.0; 1.05; 1.1; 1.5; 2.5; 3.0; 3.1; 3.1]*0.65; % z con salita lenta, poi rapida, poi quasi piatta
Q1=[x, y, z];

% parametri per la generazione della curva del profilo
w1 = ones(1,size(Q1,1));
k = 3;  % grado
...
% parametri per generare le circonferenze del becco
wQ = ones(1,size(Q,1));
tiQ=[0 0 linspace(0,1,size(Q,1)) 1 1];
nQ = size(Q,1); nP=size(P,1);
r = [linspace(1, 0.2, nQ-1) 0.05];  % raggio decrescente con restringimento finale 
for j = 1:nQ
    [P,w,ti]=circonf_inscritta(r(j),4);

    P(:,3)=1; P = P(:,[3 2 1]); % scambiando le coordinate x e z, applica una rotazione di 90°
    % Posiziona il profilo
    for i=3:-1:1
        H(:,j,i)=P(:,i)+Q(j,i);
    end
end
...
S(:,:,2)=S(:,:,2)-1; S(:,:,3)=S(:,:,3)+1.9; S(:,:,1)=S(:,:,1)-.9;
S=S*0.8;
...
% "nascondo" il bordo creato dalla generazione della superficie applicando
% una riflessione rispetto il piano XZ
...
Q_man=de_boor_razionale(Q1_man,w1_man,ti1_man);
...
theta = linspace(0, 2*pi, N+1)';  % angolo [0,2pi]
NP = 20;
